...
async function loadPublicContent(){
  ...
  const teamList=document.querySelector('#owner-team-list');teamList.replaceChildren();if(!team.length)teamList.append(empty('No team members published yet.'));team.forEach(item=>{const card=mini(`${item.name}${item.role?` · ${item.role}`:''}`,item.bio);if(item.hasPhoto){const image=document.createElement('img');image.className='content-thumbnail';image.src=`api/team/${item.id}/photo`;image.alt=item.name;card.prepend(image);}card.append(action('Remove',()=>deleteTeam(item.id),'danger'));teamList.append(card);});
}
